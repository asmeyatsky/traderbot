    ...
    def get_historical_prices(self, symbol: Symbol, start_date: date, end_date: date) -> List[Price]:
        ...
        try:
            ...
            for idx, row in filtered_data.iterrows():
                prices.append(Price(amount=float(row['4. close']), currency='USD'))
...
class PolygonAdapter(MarketDataPort):
    ...
    def get_current_price(self, symbol: Symbol) -> Optional[Price]:
        ...
        try:
            ...
            if trades:
                return Price(amount=trades.price, currency='USD')
        except Exception as e:
    ...
    def get_historical_prices(self, symbol: Symbol, start_date: date, end_date: date) -> List[Price]:
        ...
        try:
            ...
            for result in response:
                prices.append(Price(amount=result.close, currency='USD'))
    ...
    def get_current_price(self, symbol: Symbol) -> Optional[Price]:
        ...
        try:
            ...
            if not hist.empty:
                ...
                return Price(amount=float(current_price), currency='USD')
        except Exception as e:
    ...
    def get_historical_prices(self, symbol: Symbol, start_date: date, end_date: date) -> List[Price]:
        ...
        try:
            ...
            for idx, row in hist.iterrows():
                prices.append(Price(amount=float(row['Close']), currency='USD'))
...
class MarketauxAdapter(NewsAnalysisPort):
    ...
    def analyze_sentiment(self, text: str) -> NewsSentiment:
        ...
        # Marketaux typically analyzes sentiment of news articles
        # This is a simplified implementation
        # In practice, you'd need to call their API with the text
        return NewsSentiment(
            score=0,  # Placeholder - would come from API response
            confidence=0,  # Placeholder - would come from API response
            source="Marketaux"
        )
    
    def batch_analyze_sentiment(self, texts: List[str]) -> List[NewsSentiment]:
        """Analyze sentiment of multiple texts."""
        # In real implementation, this would call the API with multiple texts
        # For now, return placeholder values
        return [self.analyze_sentiment(text) for text in texts]
    
    def extract_symbols_from_news(self, news_text: str) -> List[Symbol]:
        """Extract stock symbols mentioned in news text."""
        # This would use NLP to extract ticker symbols from the text
...
class FinnhubAdapter(MarketDataPort, NewsAnalysisPort):
    ...
    def get_current_price(self, symbol: Symbol) -> Optional[Price]:
        ...
        try:
            ...
            if quote and 'c' in quote:  # 'c' is current price in Finnhub response
                return Price(amount=float(quote['c']), currency='USD')
        except Exception as e:
        ...
        try:
            ...
            if 'c' in response and len(response['c']) > 0:
                for i in range(len(response['c'])):
                    prices.append(Price(amount=float(response['c'][i]), currency='USD'))
    ...
    def analyze_sentiment(self, text: str) -> NewsSentiment:
        ...
        # Finnhub has a sentiment endpoint but it's usually for specific symbols
        # This is a simplified implementation
        return NewsSentiment(
            score=0,  # Placeholder - would come from API response
            confidence=0,  # Placeholder - would come from API response
