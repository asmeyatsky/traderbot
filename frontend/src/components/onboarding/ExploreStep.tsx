...
export default function ExploreStep({ onNext }: ExploreStepProps) {
  ...
  return (
    <div className="animate-fade-in text-center">
      <h2 className="text-2xl font-bold text-gray-900 dark:text-white">Explore AI Predictions</h2>
      ...
      <div className="mx-auto mt-6 max-w-xs space-y-3 text-left">
        <div>
          <label className="block text-xs font-medium text-gray-500 dark:text-gray-400">Market</label>
          <select
      ...
      <div className="mx-auto mt-6 max-w-sm">
        ...
        {data && !isLoading && (
          <div className="rounded-lg bg-gray-50 dark:bg-gray-800 p-6 text-left">
            <div className="text-center text-lg font-bold text-gray-900 dark:text-white">{data.symbol}</div>
            <div className="mt-4 grid grid-cols-2 gap-4 text-sm">
              ...
              <div>
                ...
                <div className="font-semibold text-gray-900 dark:text-white">{(data.confidence * 100).toFixed(0)}%</div>
              </div>
              <div>
                <div className="text-gray-500 dark:text-gray-400">Current Price</div>
                <div className="font-semibold text-gray-900 dark:text-white">{formatCurrency(data.current_price)}</div>
              </div>
              <div>
                <div className="text-gray-500 dark:text-gray-400">Predicted Price</div>
                <div className="font-semibold text-gray-900 dark:text-white">{formatCurrency(data.predicted_price)}</div>
              </div>
            </div>
          </div>
        )}
      ...
      <div className="mt-4 rounded-lg bg-indigo-50 dark:bg-indigo-900/30 p-3">
